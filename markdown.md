...
In this example, you can see that Svelte adds classes (where `xyz` is autogenerated) to both the descendant and ancestor
...
Vue, on the other hand, uses attributes and doesn't add any selector to the ancestor. So it can take advantage of the
...
than attributes, although this has been changing with some optimizations from Firefox and WebKit.)
...
In contrast, Enhance generates a selector that will have to completely rely on the Bloom filter optimization. Out of
these three, Svelte will be the most optimal.
